...
- [1. Data Communication](#chapter-1)
- [2. Computer Networks](#chapter-2)
- [3. Network Models](#chapter-3)
- [4. OSI and TCP/IP Layer Functions](#chapter-4)
- [5. World Wide Web](#chapter-5)
- [6. Network Security](#chapter-6)
- [7. Mobile Technology](#chapter-7)
- [8. Cloud Computing and IoT](#chapter-8)
- [Coverage and quality notes](#coverage-and-quality-notes)

## Status and use

This guide contains all **255 question blocks currently recoverable and assigned to Unit 9** from the local UGC NET archive. Questions are arranged chapter-wise and numbered continuously through the unit.

> [!WARNING]
> This is a working extraction inventory, not a complete solved guide. **0 answers are validated and 255 remain pending** in this unit. Some unit and chapter placements use fallback routing, and OCR or missing figures can make questions incomplete.
...
- **Core idea:** Locate the question at the correct layer, draw the frame/packet path, and label addresses, headers, windows, or signal parameters.
...
- **Rules/formulas:** Transmission delay = length/rate; propagation delay = distance/speed. Hosts per IPv4 subnet are normally 2^h-2. Nyquist and Shannon answer different channel assumptions.
- **Frequent traps:** Do not mix MAC, IP and port addresses; bandwidth with throughput; forwarding with routing; CRC polynomial division with ordinary subtraction.
...
**Exam lens:** Locate the question at the correct layer, draw the frame/packet path, and label addresses, headers, windows, or signal parameters.

**Reusable method:** Normalize bit/byte and second units. For subnetting split prefix and host bits; for error control perform modulo-2 arithmetic; for sliding windows trace sequence numbers.

**High-yield rules:** Transmission delay = length/rate; propagation delay = distance/speed. Hosts per IPv4 subnet are normally 2^h-2. Nyquist and Shannon answer different channel assumptions.

**Common traps:** Do not mix MAC, IP and port addresses; bandwidth with throughput; forwarding with routing; CRC polynomial division with ordinary subtraction.
...
*UGC NET Dec 2012, Paper II, original Q37*

> (D) 1 AES is a round cipher based on the Rijndal Algorithm that uses a 128-bit block of data. AES has three different
...
*UGC NET Dec 2015, Paper III, original Q15*

> A device is sending out data at the rate of 2000 bps. How long does it take to send a file of 1,00,000 characters ? (1) 50 2) 200 (3) 400 (4) 800
...
> The PVM communication model assumes that any task can send a message to any other PVM task and that there is no limit to the size or number of such messages. (4) In PVM model, the message order is not preserved. J-8715 10 Paper-Il
...
*UGC NET Dec 2018, original Q91*

> If the frame buffer has 10-bits per pixel and 8-bits are allocated for each of the R, G, and B components, then what would be the size of the color lookup table (LUT)? . (28 + 29) bytes Personal Exam (210 + 28) bytes 91394342362 (210 + 224) bytes 91394342363. (210 + 211) bytes 91394342364.
...
### 2. Computer Networks (3 questions)

**What to master:** Topologies; LAN, MAN and WAN; Wireless Networks; Internet.
...
### 3. Network Models (10 questions)

**What to master:** Layered Architecture; OSI and Protocols; TCP/IP Suite; Physical, Logical, Port and Specific Addresses; Switching.
...
### 4. OSI and TCP/IP Layer Functions (65 questions)

**What to master:** Framing; Error Detection and Correction; Flow and Error Control; Sliding Window; HDLC; Multiple Access - CSMA/CD, CSMA/CA, Reservation, Polling, Token Passing, FDMA, CDMA, TDMA; Devices; Backbones; VLANs; IPv4 and IPv6; Addressing; Datagram, Fragmentation and Checksum; ARP; Delivery; Routing; TCP, UDP and SCTP; Flow, Error and Congestion Control.
...
> Which of the following fields in IPv 4 datagram is not related to fragmentation? (1) Type of service
...
> Which of the following is not a congestion policy at network layer ? (1) Flow Control Policy (2) Packet Discard Policy
...
> Consider the following two statements: S1: TCP handles both congestion and flow control S2: UDP handles congestion but not flow control Which of the following options is correct with respect to the above statements (S1) and S2) ? Choose the correct answer from the code given below : Code: . Neither S1 nor S2 is correct Guide 91394342550. S1 is not correct but S2 is correct rsonal Exam 91394342551. S1 is correct but S2 is not correct 91394342552. Both, S1 and S2 are correct Single Dine Questio 0 on: No Opion Oriens: Vern aye: MC Option Shufling: Yes
...
> Identify the correct sequence in which the following packets are transmitted on the network by a host when a browser requests a webpage from a remote server, assuming that the host has just been restarted. Options : HTTP GET request, DNS query, TCP SYN 91394342553. DNS query, HTTP GET request, TCP SYN 91394342554. 91394342555. TCP SYN, DNS query, HTTP GET request 91394342556. DNS query, TCP SYN, HTTP GET request ingle Line Question Option: No
...
> You are designing a link layer protocol for a link with bandwidth of 1 Gbps (10° bits/second) over a fiber link with length of 800 km. Assume the speed of light in this medium is 200000 km/second. What is the propagation delay in this link? 1. 1 millisecond 2. 2 milliseconds 3. 3 milliseconds 4 milliseconds 64635085868.2 64635085869.3 64635085870.4 Single Line Question 0 lion: No Opion Oriena o: Verticaype : MCQ Option Shufling: No
...
> In the TCP/IP model, encryption and decryption are functions of layer. data link 2. network transport 4. application 64635085880. 2 64635085881. 3 64635085882. 4
...
> What percentage (%) of the IPv4, IP address space do all class C addresses consume? 12•5% 25% 3. 37•5% repp + 50% ersonal Exam Guide. 64635085888. 2 64635085889.3 64635085890.4
...
> Output Feedback Mode 2. User Datagram Protocol (UDP) 3. Routing Protocol (RP) 4. Datagram Control Protocol (DCP)
...
> Based on the following passage, answer the Questions: A 3000 km long trunk operates at 1.536 mbps and is used to transmit 64 bytes frames and uses sliding window protocol. The propagation speed is 6 u sec/km. The sender window size to get the maximum efficiency is 1. 108 2. 109 55 4. 56
...
> Consider the three way hand shaking process followed during TCP connection establishment between two hosts A and B. Let S and R be two random 32-bit starting sequence numbers chosen by A and B, respectively. Suppose A sends a TCP segment having SYN bit =1, SEQ number=S and ACK bit = 0 and B accepts the connection request. Which one of the following choices represents the information present in the TCP segment header that is sent by B to A? (1) SYN bit = 1, SEQ number =S+1, ACK bit=0, ACK number =R, FiN bit=0 (2) SYN bit=0, SEQ number = R, ACK bit=1, ACK number=S + 1, FiN bit =0 SYN bit =1, SEQ number= R, ACK bit=1, ACK number=S + 1, FiN bit=0 (4) SYN bit =1, SEQ number= R, ACK bit =1, ACK number=S, FiN bit=0
...
> TOMA CO C. Random-access protocol I11. Reservation D. Controlled-access protocol IV. HDIC CO Choose the correct answer from the options given below A-II. B-IV, C-I, D-III (1) A-L, B-MI. C-IV, D-IL What is the total delay (latency) for a frame size of 10 million bits that is being set up on a link with (2) A-III, BIV, C-I, D-II A-IV, B-III, C-II, D-I (4) (3) 20 routers, each having a queuing time of 2 us and a processing time of 1 us? The length of link The speed of light inside the link is 2 x 108m/s. The link has bandwidth of 6 Mbps.
...
### 5. World Wide Web (11 questions)

**What to master:** URL; DNS and Resolution; Electronic Mail Architecture; SMTP, POP and IMAP; TELNET; FTP.
...
> What is the name of the protocol that allows a client to send a broadcast message with its MAC address and receive an IP address in reply? 1. ARP DNS RARP 4. ICMP 64635085864.2 64635085865.3 64635085866.4 Single Line Question 03 on: No Opion Orientation: Veiicaype: MCQ Option Shufling: No
...
> Which mode is a block cipher implementation as a self synchronizing stream cipher? 1. Cipher Block Chaining Mode 2. Cipher Feedback Mode 3. Electronic Codebook Mode
...
### 6. Network Security (44 questions)

**What to master:** Malware; Cryptography; Steganography; Secret-Key and Public-Key Algorithms; Digital Signatures; VPNs; Firewalls.
...
> A network with bandwidth of 10 Mbps can pass only an average of 15,000 frames per minute with each frame carrying an average of 8,000 bits. What is the throughput of this network ? (1) 2 Mbps 60 Mbps (3) 120 Mbps (4) 10 Mbps
...
> Which of the following statement/s is/are true? (i) Firewalls can screen traffic going into or out of an organization. (ii) Virtual private networks can simulate an old leased network to provide certain desirable properties. Choose the correct answer from the code given below : Code: . (i) only (ii) only 91394342462. 91394342463. Both (i) and (ii) 91394342464. Neither (i) nor (ii)
...
> Consider the following properties with respect to a flow network G = (V,E) in which a flow is a real-valued function f:V×V →R: P, : Forall u, vEV, f(u, v) =-f(v, u) P2 : f (u, v) = 0 for all u EV Which one of the following is/are correct? 1. Only P, Ai Only P, 3. Both P, and Pa 'ersonal Exam Guide. + Neither P, nor P, 64635085812.2 64635085813.3 64635085814. 4
...
> A fully connected network topology is a topology in which there is a direct link between all pairs of nodes. Given a fully connected network with n nodes, the number of direct links as a function of n can be expressed as n(n+1) 1. 2 (n+ 1) 2 n 2 - 4. n(n-1) 2 64635085860.2
...
> Which of the following is an example of unsupervised neural network? Back-propagation network 2. Hebb network 3. Associative memory network Self-organizing feature map 64635085928. 2 64635085929. 3 64635085930.4
...
> Consider a CSMA/CD network that transmits data at the rate of 100 Mbps over a 1 Kilometre cable with no repeater. If the minimum frame size required for this network is 1250 bytes, what is the signal speed (km/sec) in the cable. 8000 (2) 16000 (3) 12000 (4) 20000
...
> Which of the followings are type of interface in de AT network? network-to-network A. user-to-user B. user-to-network Choose the correct answer from the options given below : D. host-to-network User-to-host (3) Cand DOlly (4) Dand E Only
...
### 7. Mobile Technology (9 questions)

**What to master:** GSM and CDMA; GSM Services and Architecture; Mobile Computing; Middleware and Gateways; Mobile IP and Protocols; Satellites; Wireless Networks, Topologies and Transmission; Cellular Topology; MANETs; WLANs; Geolocation; GPRS; SMS.
...
### 8. Cloud Computing and IoT (73 questions)

**What to master:** SaaS, PaaS and IaaS; Public and Private Clouds; Virtualization; Virtual Servers; Cloud and Database Storage; Resource Management; SLA; IoT Basics.
...
> The number of bit strings of length (C) 300 K eight that will either start with a 1 bit (D) 600 K or end with two bits 00 shall be (A) 32
...
> Suppose that someone starts with a chain letter. Each person who receives the letter is asked to send it on to 4
...
> Match the following : from position pi, position p2 a. TTL
...
> The redundancy in images stems from : (1) pixel decorrelation 2) pixel correlation (3) pixel quantization (4) image size J-8715 12 Paper-III
...
> Consider a vocabulary with only four propositions A, B, C and D. How many models are there for the following sentence ? -AV-BV→CV→D Options : Guide 91394342589. 7 Personal Exam 91394342590. 8 91394342591. 15 91394342592. 16 Single one Question 4 on siNo Opton Orienta : We aye: MC Option Shufling : Yes
...
> How many bit strings of length ten either start with a 1 bit or end with two bits 00? 320 xam 480 3. 640 Persona 4. 768 Options : 'our 64635085547.1 64635085548. 2 64635085549.3 64635085550.4
...
> Match List-l with List-ll : List - 11 A. Channelization protocol List - 1 ALOHA B. Bit-oriented protocol
...
- Recovered question blocks in this unit: **255**
- Chapter placements with direct keyword support: **186**
